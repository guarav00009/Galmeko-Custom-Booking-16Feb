...
class StatusFilter(SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        if self.value():
            return queryset.select_related().filter(user__status=self.value())
        else:
            return queryset
...
class HospitalAdmin(admin.ModelAdmin):
    ...
    model = Hospital
    list_display = ('user_name', 'email', 'hospital_name',
                    'registration_no', 'address', 'phone', 'file_link', 'Action')
    list_filter = (StatusFilter,)
    list_per_page = 5

    fieldsets = (
        (None, {'fields': ('hospital_name', 'registration_no', 'address')}),
    )
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('hospital_name', 'registration_no', 'address')}
         ),
    )
